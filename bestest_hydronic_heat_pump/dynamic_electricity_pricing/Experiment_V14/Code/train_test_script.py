...
"""
import os
os.chdir(r'C:/Users/gauthambekal93/Research/model_based_rl/bestest_hydronic_heat_pump/dynamic_electricity_pricing/Experiment_V14/Code')
...
from model_architecture_env_policy import Environment_Module, Policy_Module, Policy_Module_Pretrained_Boptest
...
policy_pretrained_boptest = Policy_Module_Pretrained_Boptest(int(env_attributes["state_space"]),  int(env_attributes["action_bins"]), int(env_attributes["h_size"]), device, env_attributes["no_of_action_types"] - 1).to(device) #was  int(env_attributes["action_space"])
...
policy_pretrained_boptest.load_state_dict(checkpoint['model_state_dict'])
...
source_model_state_dict = policy_pretrained_boptest.state_dict()

# Create a new state_dict for the target model
target_model_state_dict = policy.state_dict()
...
for name, param in source_model_state_dict.items():
    if name != 'fc4':
        ...
        print(f"{name} not found in target model, skipping this layer.")
    ...
    for _ in range(policy_update_loop):
        ...
        for t in range(max_t): #was 672 
            ...
            if ( t % actual_env_train_step == 0) or (only_use_actual_env):
                  ...
                  new_state, reward = Y_pred[:,:6], Y_pred[:,6:]
