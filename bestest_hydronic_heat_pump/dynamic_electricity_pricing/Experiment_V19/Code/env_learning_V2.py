...
from env_model_architecture_V2 import Bayesian_Env_Model
...
class train_environment:
    ...
    def __init__(self, model_type ):
        ...
        self.epochs, self.batch_size = 3000, 100  #seems like we should have more epochs for further training
        
        self.model = Bayesian_Env_Model(input_dim, output_dim, latent_dim , hidden_dim)
        
        self.optimizer = optim.Adam( self.model.parameters(), lr= 0.0001 )     #wad lr= 0.0001 
    ...
    def calculate_performance_metrics(self):  
        ...
        #mu_z, std_z = self.model.forward_encoder(a)
        ...
        #mu_z, std_z = self.model.forward_encoder(self.test_x)
        tmp =  torch.cat([torch.ones( self.test_x[:,:11].shape), self.test_x[:, 11:] ], dim = 1)
        mu_z2, std_z2 = self.model.forward_encoder(tmp)
        
        z_samples2 = self.model.sample_latent_variable(mu_z2, std_z2, no_of_z_samples = 1000)  #this will be of shape (batch_size, no. of samples, z dim)
        
        mu_y_samples2, _  = self.model.forward_decoder(  z_samples2 )
        
        mu_y2 = torch.mean(mu_y_samples2, dim = 1) 
        
        uncertanity2 = torch.mean( torch.std(mu_y_samples2, dim = 1), dim =0 )  
        
        mse2 = nn.MSELoss()(mu_y2, self.test_y) 
        
        
        mu_z3, std_z3 = self.model.forward_encoder(self.train_x)
        
        z_samples3 = self.model.sample_latent_variable(mu_z3, std_z3, no_of_z_samples = 1000)  #this will be of shape (batch_size, no. of samples, z dim)
        
        mu_y_samples3, _  = self.model.forward_decoder(  z_samples3 )
        
        mu_y3 = torch.mean(mu_y_samples3, dim = 1) 
        
        uncertanity3 = torch.mean( torch.std(mu_y_samples3, dim = 1), dim =0 )  
        
        mse3 = nn.MSELoss()(mu_y3, self.train_y) 
