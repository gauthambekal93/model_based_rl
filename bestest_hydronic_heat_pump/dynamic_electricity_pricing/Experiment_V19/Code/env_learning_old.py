...
class Bayesian_Env_Model(nn.Module):
    ...
    def calculate_metrics(self, batch_data):  
        ...
        for data in batch_data:
            ...
            z_dist = self.sample_latent_variable(mu_encoder, std_encoder)  #has 1000 as 3rd argument
...
def initialize_trajectory():
    ...
    state = env.reset()[0]
    state = state[ 1:]  #for now we dont consider the time component
    return actor, state


    
def generate_trajectory_data( state,  kl_weight, len_of_trajectories = 200, max_trajectories = 1000):
    
    
    def run_trajectories(state,time_step, no_of_trajectories):
        
        #if time_step>1:
        #    print("Time step: ", time_step, "State: \n ",state[:,0], state[:,-1] )
        ...
        mu_encoder, std_encoder =  env_model.forward_encoder( env_input )
        
        z_dist = env_model.sample_latent_variable(mu_encoder, std_encoder, no_of_trajectories)  
        
        mu_decoder, _  = env_model.forward_decoder(  z_dist )
        
        next_state = torch.cat( [ mu_decoder[:, 0:1 ], state[:, 2:].repeat(no_of_trajectories, 1),  mu_decoder[:, 1:2 ] ], dim = 1)  #next_state, future_dry_temp in crrent time, dry_bulb_temp
        
        uncertanity =  mu_decoder.std(dim = 0).detach().clone()
        
        return next_state, uncertanity
        
    
    #initialize_trajectories    
    state, uncertanity = run_trajectories(state, time_step = 1 , no_of_trajectories = max_trajectories) #here we are sampling multiple z's from a single z distribution
    print("Time step: ",1, "Uncertanity: ", uncertanity)
    
    #run trajectories
    for t in range(2, len_of_trajectories):
        state, uncertanity = run_trajectories(state, t, no_of_trajectories = 1)   #here we are sampling only one z per distribution but we have multiple such z distributions
...
if __name__ == "__main__":
    ...
    generate_trajectory_data(state, kl_weight)
