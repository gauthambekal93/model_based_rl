...
"""
import os
os.chdir(r'C:/Users/gauthambekal93/Research/model_based_rl/bestest_hydronic_heat_pump/dynamic_electricity_pricing/Experiment_V15/Code')
...
env_model_pretrained_boptest = Environment_Module(int(env_attributes["state_space"]),   int(env_attributes['no_of_action_types']) - 1, alpha=1, beta=0.5, model_type = 'pre-trained') #was  int(env_attributes["action_space"])
...
env_model_pretrained_boptest.load_state_dict(checkpoint['model_state_dict'])


env_model = Environment_Module(int(env_attributes["state_space"]), n_actions=  int(env_attributes['no_of_action_types']) , alpha=1, beta=0.5, model_type = 'fine-tuned') #beta was 0.2

env_model_optimizer = optim.Adam(env_model.parameters(), lr=float(0.0001 ) ) #was 0.001



source_model_state_dict = env_model_pretrained_boptest.state_dict()

# Create a new state_dict for the target model
target_model_state_dict = env_model.state_dict()

# Update the target model state_dict with specific layers from the source model
for name, param in source_model_state_dict.items():
    if ('input_layer_next_state' in name ) or ( 'input_layer_reward' in name):
        print(f"skipping {name} from target model")
    else:
        target_model_state_dict[name] = param
        

# Load the updated state_dict into the target model
env_model.load_state_dict(target_model_state_dict)
...
compare_models(env_model_pretrained_boptest  ,env_model)
...
actual_env_train_step = 50 #was 20  #was 10
