...
def train_hypernet(target_model, hypernet, hypernet_optimizer, env_memory, task_index, epochs, hypernet_old = None):
    ...
    train_X, train_y, test_X, test_y = data_processing(env_memory) #data_processing2() 
    ...
    for epoch in range(epochs):
        ...
        for batch_no in range( 0, int(len(train_X) ), batch_size ):
            ...
            loss.backward()   #I THINKERROR IS BEFORE THIS LINE SINCE THIS CAUSED HUGE GRADIENT
...
def collect_from_learnt_env(env_memory, hypernet, target_model, task_index, sample_size = 1):
      
      state_samples, _ , _, _, _ =  env_memory.sample_memory( sample_size )
      
      action = random(state_samples)
      #WE ALSO NEED TO CAPTURE UNCERTANITY HERE !!!
      input_data = torch.cat( [ torch.tensor( state_samples ).reshape(1,-1) , action.detach().clone()  ]  , dim = 1)
      ...
      final_predictions = torch.mean(final_predictions , dim =1 ).reshape(-1,1)
      
      next_state_pred = torch.cat( [ state_samples[0], final_predictions[0], state_samples[2:], final_predictions[1]], dim =1 )
      
      reward = final_predictions[3]
      
      return state_samples , action, reward, next_state_pred 
